...
first row is the query itself, and raises an `IndexError` when a query
has fewer than two comparisons. Here a query with no comparisons gets
`NA` for both, and one with a single comparison gets `NA` for
`nextBest`.
